...
func NewSessionsModel(db *sql.DB) *SessionsModelType {
...
func (model *SessionsModelType) InsertSession(data SessionsRow) (int, error) {
	...
	row := model.Db.QueryRow(query, data.UserID, data.Model, data.PromptTokens, data.CompletionTokens, data.TotalTokens)
...
func (model *SessionsModelType) UpdateSession(data SessionsRow) (*SessionsRow, error) {
	...
	_, err := model.Db.Exec(query, data.PromptTokens, data.CompletionTokens, data.TotalTokens, data.SessionID)
	if err != nil {
