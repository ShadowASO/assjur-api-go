...
import (
	...
	"ocrserver/internal/grpc/pb/authpb"
...
func (c *ClientAuth) Ping(ctx context.Context) error {
	...
	resp, err := c.client.Health(rpcCtx, &authpb.HealthRequest{})
	if err != nil {
		return normalizeGRPCError(err, "auth-srv indisponível")
	}

	if resp == nil || !resp.GetOk() {
		return errors.New("auth-srv retornou health inválido")
	}

	if strings.TrimSpace(resp.GetStatus()) != "serving" {
		return fmt.Errorf("auth-srv não está pronto: status=%q", resp.GetStatus())
	}

	mslogger.LoggerGlobal.Info("serviços de autenticação 'auth-srv' respondendo normalmente")
