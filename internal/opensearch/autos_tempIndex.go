...
type Autos_tempIndexType struct {
	osCli     *opensearchapi.Client
	indexName string
}

// Novo cliente para o índice autos
func NewAutos_tempIndex() *Autos_tempIndexType {
...
type BodyAutos_tempIndex struct {
...
type ResponseAutos_tempIndex struct {
	Id     string `json:"id"`
	IdCtxt int    `json:"id_ctxt"`
	IdNatu int    `json:"id_natu"`
	IdPje  string `json:"id_pje"`
...
type searchResponseAutos_tempIndex struct {
	Hits struct {
		Hits []struct {
			ID     string                  `json:"_id"`
			Source ResponseAutos_tempIndex `json:"_source"`
		} `json:"hits"`
	} `json:"hits"`
}

func (idx *Autos_tempIndexType) Indexa(
	IdCtxt int,
	IdNatu int,
	IdPje string,
	Doc string,
	idOptional string,
) (*consts.Autos_tempRow, error) {

	// Monta o documento para indexar
	doc := BodyAutos_tempIndex{
		IdCtxt: IdCtxt,
		IdNatu: IdNatu,
		IdPje:  IdPje,
		Doc:    Doc,
...
func (idx *Autos_tempIndexType) Update(
...
func (idx *Autos_tempIndexType) ConsultaById(id string) (*consts.Autos_tempRow, error) {
...
func (idx *Autos_tempIndexType) ConsultaByIdCtxt(idCtxt int) ([]consts.Autos_tempRow, error) {
	if idx.osCli == nil {
	...
	docs := make([]consts.Autos_tempRow, 0, len(result.Hits.Hits))
	for _, hit := range result.Hits.Hits {
		var row consts.Autos_tempRow
...
func (idx *Autos_tempIndexType) ConsultaByIdNatu(idNatu int) ([]consts.Autos_tempRow, error) {
	if idx.osCli == nil {
		logger.Log.Error("Erro: OpenSearch não conectado.")
		return nil, fmt.Errorf("erro ao conectar ao OpenSearch")
	}
...
func (idx *Autos_tempIndexType) ConsultaSemantica(vector []float32, idNatuFilter int) ([]ResponseAutosIndex, error) {
	if idx.osCli == nil {
		logger.Log.Error("Erro: OpenSearch não conectado.")
		return nil, fmt.Errorf("erro ao conectar ao OpenSearch")
	}
...
func (idx *Autos_tempIndexType) IsExiste(idCtxt int, idPje string) (bool, error) {
	if idCtxt <= 0 || idPje == "" {
